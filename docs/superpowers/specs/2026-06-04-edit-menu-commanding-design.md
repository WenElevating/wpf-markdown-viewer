...
- Do not create `EditMenuView`, `EditMenuViewModel`, or a Sample-layer command catalog for the first version.
...
| Paste Image | none | Route to an editor command that uses existing clipboard image/file paste behavior and inserts Markdown image syntax. |
...
- Normalize clipboard plain-text insertion.

The helper should not know about `MainWindow`, dialogs, recent files, status text, or localization.
...
Top Edit menu button
  ...
  -> CommandTarget = editor TextBox or MarkdownEditor
...
Top Edit menu button
  ...
  -> CommandTarget = MarkdownEditor
  -> MarkdownEditor CommandBinding CanExecute/Executed
  -> EditorTextOperations computes mutation
  -> TextBox text updates in one undo unit
  -> MarkdownEditor.MarkdownChanged
```
...
Add `MainWindow.*` localization keys for top Edit menu entries. Existing `Editor.*` keys remain for the `MarkdownEditor` context menu unless the same string is clearly reusable.
...
- `CommandCanExecute_ClipboardOrSelectionState_ReflectsAvailability`
